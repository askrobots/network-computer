...
func streamAudio(ctx context.Context, device string, track sampleWriter) {
	for ctx.Err() == nil {
		...
		// One Opus packet per RTP packet: an Ogg page can hold two, and a page
		// sent whole is not a valid Opus packet (the receiver decodes garbage).
		ogg := oggopus.NewReader(stdout)
		// Pace packets on their own clock. Capture arrives in clumps; sent as it
		// came, the receiver saw packets bunch up then pause and kept speeding up
		// and slowing down playback to cover it (a warbly, old-streaming sound).
		var start time.Time
