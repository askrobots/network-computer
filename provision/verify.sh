...
check "live streams ready (MediaMTX $(/usr/local/bin/mediamtx --version 2>/dev/null), nc-stream)" sh -c 'test -x /usr/local/bin/mediamtx && test -f /etc/nc/mediamtx.yml && test -x /usr/local/bin/nc-stream && id nc-streams >/dev/null'
check "live streams: only SRT is reachable from outside, and only with a passphrase" sh -c 'systemctl is-active --quiet nc-streams || exit 0; test "$(grep -c "^MTX_PATHDEFAULTS_SRTPUBLISHPASSPHRASE=.\{10,\}" /etc/nc/streams.env)" = 1 && ! ss -Hltn | awk "{print \$4}" | grep -E ":(9997|9998|9996|8554|8888|8889)$" | grep -vqE "^(127\.0\.0\.1|\[::1\]):"'
