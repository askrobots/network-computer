...
case "$1" in
  ...
  provision) IP=$(ip); ssh root@"$IP" 'mkdir -p /root/provision'; scp -q -r "$ROOT/provision/." root@"$IP":/root/provision/; ssh root@"$IP" "NC_PUBLIC_IP=$IP NC_HOST_NAME=$NAME sh /root/provision/apply.sh" ;;
